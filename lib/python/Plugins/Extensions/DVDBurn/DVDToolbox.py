		...
		for line in mediuminfo.splitlines():
			if line.find("Free Blocks:") > -1:
				...
				if line.find("blank") > -1:
					print("[DVDBurn] dvd+rw-mediainfo Disc status blank capacity=%d, used=0" % (capacity))
					capacity = used
					used = 0
				elif line.find("complete") > -1 and formatted_capacity == 0:
					print("[DVDBurn] dvd+rw-mediainfo Disc status complete capacity=0, used=%d" % (capacity))
					used = read_capacity
					capacity = 1
				else:
